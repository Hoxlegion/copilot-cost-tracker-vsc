<script lang="ts">
  ...
  $: forecastVisible = periodAggregate.turns >= 50 || periodCredits >= 0.5;
  $: forecastOverage = projectedPeriodCredits - budgetCredits;
  
  $: tokenDensity = (() => {
    const totalTokens = periodAggregate.costUsd > 0 
      ? (periodAggregate.costUsd * 1000000)
      : 0;
    return periodCredits > 0 ? (totalTokens / periodCredits) : 0;
  })();
...
<div class="budget-tab">
  <div class="stat-row">
    ...
    <StatCard 
      ...
      value={Math.max(0, budgetCredits - periodCredits).toFixed(0)}
    ...
    <StatCard 
      ...
      sub={periodCredits > 0 ? 'tokens per credit in range' : 'no credits in range'}
    />
  </div>
